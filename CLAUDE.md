...
npm run store:login          # sign in to the Web Store dashboard (manual, once)
npm run store:draft          # upload package + screenshots, fill the listing draft
...
**Store builds.** Both committed manifests carry a `key`, which pins the extension ID for unpacked loads. The Chrome Web Store derives the ID from the key it holds for the listing and rejects an upload whose manifest key disagrees, so `build.sh` strips `key` for the plain `prod` target only — every other target keeps it. The packaging step zips the *contents* of the build folder, because the store requires `manifest.json` at the archive root; it prefers the `zip` binary and falls back to bsdtar (via the explicit `System32` path, since Git Bash shadows it with GNU tar, which cannot write zips). `scripts/verify-package.js` gates a release on the rest: manifest references that resolve, no leftover `key`, no source maps, MV3, field lengths, and `package.json`/manifest version agreement. `store-assets/README.md` is the paste-ready listing draft — copy, permission justifications, data disclosures, reviewer notes and the submission checklist — and `store-assets/screenshots/` holds the 1280×800 captures, regenerated by `node scripts/render-store-screenshots.js` (which drives the real extension out of `build/debug` against the e2e wallet in `test-results/session`, so the shots never show a real balance). `PRIVACY.md` is the policy the dashboard's mandatory privacy-policy URL must point at.

**Store automation.** `scripts/store/` drives the Developer Dashboard: `store:login` starts Chrome on it using a dedicated gitignored profile (`.store-profile/`) and waits while a human signs in — it never handles credentials — and `store:draft` then attaches to that same window to upload the package and screenshots and fill the listing from `store-assets/listing.json` (the machine-readable twin of `store-assets/README.md`; edit the JSON first).

Two constraints shape that design. Chrome refuses to let *extensions* script the Web Store gallery at all, so an in-browser agent cannot do this and CDP is the only way in. And Chrome must not be launched *by* Playwright for the sign-in: `launchPersistentContext` applies `--enable-automation`, and Google rejects the sign-in with "This browser or app may not be secure". So `lib.js` spawns Chrome itself with only `--remote-debugging-port` — enabling an API rather than suppressing a check — and `attach()` connects over CDP afterwards. Deliberately no anti-detection flags: defeating a sign-in protection on a real account is not something to automate around, and the fallback is a manual upload. If attach fails it is nearly always another Chrome holding the profile, since a second launch just forwards the URL to the running instance.

`store:draft` stops at "save draft" and never submits for review, and any step that cannot find its target dumps a screenshot and the page HTML to `store-assets/debug/` rather than clicking blind — the dashboard DOM is unversioned and moves without notice, so the first run wants watching.
...
- `chains.ts` — the registry (`CHAINS` + per-chain `CHAIN_CONFIG_*` exports). Provider assignment: Monad + Monad testnet + BSC/Amoy → Etherscan V2 multichain (`api.etherscan.io/v2/api?chainid=N`, one free key covers all of them — chain 143 is the monadscan backend); Ethereum/Base/Polygon/Arbitrum + the Sepolias → Blockscout instances (no key); Avalanche → Routescan (etherscan-compatible, no key). `getDataProviderPresets(chainId)` returns the selectable providers for a chain — committed default first, then Etherscan V2 (every chain in this registry is in its chain list), BlockVision for the Monad chains only, then `none`. It is what the Developer Settings picker renders, so new alternatives are added there rather than hardcoded in the UI.
  ...
  The eleven original chains are written out longhand as `CHAIN_CONFIG_*`; the wider EVM mainnet set that follows them is one row per chain in `CHAIN_ROWS`, expanded by `buildChain` into `ADDITIONAL_CHAINS` and spread into `CHAINS`. Add a chain by adding a row — the builder fills in the boilerplate and derives `swapSupport` from whether the row carries a `swapSlug`. Only `CHAIN_CONFIG_{ETHEREUM,MONAD,BSC,SEPOLIA}` are imported elsewhere, so table-driven entries need no named export. Everything in a row is verified against the provider before committing: `eth_chainId` for the RPCs, the presence of `baseFeePerGas` for `gasPriceType`, `/api/v2/addresses/{a}/token-balances` for Blockscout instances, `api.etherscan.io/v2/chainlist` for the Etherscan V2 ones, `coins.llama.fi/chains` for `llamaSlug`, and OpenOcean's `/{slug}/tokenList` for `swapSlug`. Note `nativeCoingeckoId` is the *gas* coin, which is usually not the chain's headline token (gas on Optimism and Linea is ETH, not OP or LINEA); `llamaKey` prefers it over the legacy `NATIVE_COINGECKO_IDS` map in `src/lib/priceproviders/defillama.ts`. Adding a chain also means adding its `chain_key` to the `Chain` union in `src/shared/types/Chain.tsx`.
...
Network badges are generated the same way, by `node scripts/render-chain-icons.js`, which writes `src/assets/images/chain-{key}.png` from the `BADGES` table in that script (monogram plus brand colour, ink or white picked by luminance). These are deliberately not official logos — vendoring third-party marks would mean redistributing assets the project has no licence for — so they read as distinct at 24px without pretending to be the real thing. Replace one by dropping a real logo into `src/assets/images/` and repointing its `Images.tsx` entry. The chains that predate the script keep their existing brand logos.

There is no login. The route gate (`src/ui/pages/Authenticated.tsx`) is `isUnlocked && completedOnboarding`; onboarding is OnboardingHome → CreatePinCode → create/import/hardware → WalletSuccess. Earn lists only on-chain liquid staking (aPriori/Kintsu/FastLane on Monad, from `defaults.ts`).

**Navigation.** `src/ui/components/BottomNav.tsx` is the persistent bottom bar for four destinations — Home (`/`), Swap, Activity, Settings. It is rendered by each of those four pages rather than by the router, so deeper screens (send flow, coin detail, dapp confirmations) stay full height; a page that forgets to render it simply has no nav. The active item comes from `useLocation`, which means any test that renders one of those pages needs a Router in scope or a `useLocation` mock. Home itself holds two sub-tabs (Tokens, Collectibles) plus the balance hero and the action capsule. **Explore and Activity are routes, not tabs** — Explore was formerly one of two bottom tabs and is now reached from the chart icon in the Home top bar (`data-testid="open-explore"`), and transaction history was formerly Home's third sub-tab and is now `src/ui/pages/Activity.tsx`, which renders `TransactionPortfolio` in its `grouped` mode (day headings plus a timeline rail).
