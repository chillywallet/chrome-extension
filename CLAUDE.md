...
**Store automation.** `scripts/store/` drives the Developer Dashboard with Playwright: `store:login` opens it in a dedicated gitignored Chrome profile (`.store-profile/`) and waits while a human signs in — it never handles credentials — and `store:draft` then reuses that session to upload the package and screenshots and fill the listing from `store-assets/listing.json` (the machine-readable twin of `store-assets/README.md`; edit the JSON first). This works only because Playwright drives Chrome over CDP: Chrome refuses to let *extensions* script the Web Store gallery at all, which is why the in-browser agent cannot do it. It launches real Chrome via `channel: 'chrome'`, since Google is far likelier to complete a sign-in there than in bundled Chromium. `store:draft` deliberately stops at "save draft" and never submits for review, and any step that cannot find its target dumps a screenshot and the page HTML to `store-assets/debug/` rather than clicking blind — the dashboard DOM is unversioned and moves without notice, so the first run wants watching.
