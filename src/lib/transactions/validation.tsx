...
import { TransactionEnvelopeType, TransactionParams } from '../../shared/types/Transaction';
...
export async function validateTransactionOrigin(
    ...
    if (origin === ORIGIN_CHILLY) {
        ...
        if (!selectedAddresses.some(_address => isEqualCaseInsensitive(_address, from))) {
    ...
    if (!permittedAddresses.some(_address => isEqualCaseInsensitive(_address, from))) {
 ...
 */
function validateEIP1559Compatibility(txParams: TransactionParams, isEIP1559Compatible: boolean) {
...
function validateParamFrom(from: string) {
    if (!from || typeof from !== 'string') {
        throw rpcErrors.invalidParams(`Invalid "from" address ${from}: not a string.`);
    }
...
function ensureProperTransactionEnvelopeTypeProvided(
    ...
    switch (field) {
        ...
        case 'maxPriorityFeePerGas':
            if (txParams.type && txParams.type !== TransactionEnvelopeType.feeMarket) {
                throw rpcErrors.invalidParams(
 ...
 */
function ensureFieldIsString(txParams: TransactionParams, field: GasFieldsToValidate) {
