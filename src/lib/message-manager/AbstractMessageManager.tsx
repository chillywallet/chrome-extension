...
// Convert to a `type` in a future major version.
// eslint-disable-next-line @typescript-eslint/consistent-type-definitions
export interface AbstractMessageParamsInternal extends AbstractMessageParams {
    internalId?: string;
}
...
> extends BaseControllerV1<BaseConfig, MessageManagerState<M>> {
     ...
     */
    protected addRequestToMessageParams<MessageParams extends AbstractMessageParams>(
     ...
     */
    protected createUnapprovedMessage<MessageParams extends AbstractMessageParams>(
        messageParams: MessageParams,
        type: ApprovalType,
        req?: OriginalRequest,
    ) {
    ...
    approveMessage(messageParams: PM): Promise<P> {
        ...
        // @ts-ignore
        this.setMessageStatusApproved(messageParams.internalId);
    ...
    async waitForFinishStatus(
        messageParamsWithId: AbstractMessageParamsInternal,
        messageName: string,
    ): Promise<string> {
        const { internalId: messageId, ...messageParams } = messageParamsWithId;
            ...
            this.hub.once(`${messageId}:finished`, (data: AbstractMessage) => {
                switch (data.status) {
                    ...
                    case 'rejected':
                        return reject(
                            new Error(
                                `Chilly ${messageName} Signature: User denied message signature.`,
                            ),
                        );
                    case 'errored':
                        return reject(
                            // TODO: Either fix this lint violation or explain why it's necessary to ignore.
                            // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
                            new Error(`Chilly ${messageName} Signature: ${data.error}`),
                        );
                    default:
                        return reject(
                            new Error(
                                `Chilly ${messageName} Signature: Unknown problem: ${JSON.stringify(
                                    messageParams,
                                )}`,
                            ),
                        );
