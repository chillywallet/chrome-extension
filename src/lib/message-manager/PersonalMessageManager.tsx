...
// Convert to a `type` in a future major version.
// eslint-disable-next-line @typescript-eslint/consistent-type-definitions
export interface PersonalMessageParamsInternal extends AbstractMessageParamsInternal {
    data: string;
}
...
> {
    ...
    prepMessageForSigning(
        messageParams: PersonalMessageParamsInternal,
    ): Promise<PersonalMessageParams> {
        // Using delete operation will throw an error on frozen messageParams
        const { internalId: _internalId, ...messageParamsWithoutId } = messageParams;
