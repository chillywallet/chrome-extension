...
// Convert to a `type` in a future major version.
// eslint-disable-next-line @typescript-eslint/consistent-type-definitions
export interface EncryptionPublicKeyParamsInternal extends AbstractMessageParamsInternal {
    data: string;
}
...
export class EncryptionPublicKeyManager extends AbstractMessageManager<
    ...
    EncryptionPublicKeyParamsInternal
> {
    /**
            ...
            this.hub.once(`${messageId}:finished`, (data: EncryptionPublicKey) => {
                switch (data.status) {
                    ...
                    case 'rejected':
                        return reject(
                            new Error(
                                'Chilly EncryptionPublicKey: User denied message EncryptionPublicKey.',
                            ),
                        );
                    default:
                        return reject(
                            new Error(
                                `Chilly EncryptionPublicKey: Unknown problem: ${JSON.stringify(
                                    messageParams,
                                )}`,
                            ),
                        );
    ...
    prepMessageForSigning(
        messageParams: EncryptionPublicKeyParamsInternal,
