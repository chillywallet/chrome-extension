...
import { ConsoleLike, Maybe, getRpcPromiseCallback, isValidChainId } from './utils';
...
export abstract class BaseProvider extends SafeEventEmitter {
     ...
     * If null, Chilly is either locked or the user has not permitted any
     ...
     * @param initialState.isUnlocked - Whether the user has unlocked Chilly.
    ...
    protected _rpcRequest(
        ...
        if (!Array.isArray(payload)) {
            ...
            if (payload.method === 'eth_accounts' || payload.method === 'eth_requestAccounts') {
                ...
                callbackWrapper = (error: Error, response: JsonRpcSuccess<string[]>) => {
    ...
    protected _handleChainChanged({
        chainId,
    }: { chainId?: string | undefined; networkVersion?: string | undefined } | undefined = {}) {
    ...
    protected _handleAccountsChanged(accounts: unknown[], isEthAccounts = false): void {
        ...
        if (!Array.isArray(accounts)) {
            this._log.error(
                'Chilly: Received invalid accounts parameter. Please report this bug.',
                accounts,
            );
            _accounts = [];
        }
        ...
        for (const account of accounts) {
            if (typeof account !== 'string') {
                this._log.error(
                    'Chilly: Received non-string account. Please report this bug.',
                    accounts,
                );
                _accounts = [];
                break;
        ...
        if (!dequal(this._state.accounts, _accounts)) {
            ...
            if (isEthAccounts && this._state.accounts !== null) {
                this._log.error(
                    `Chilly: 'eth_accounts' unexpectedly updated accounts. Please report this bug.`,
                    _accounts,
                );
            }
    ...
    protected _handleUnlockStateChanged({
        ...
        if (typeof isUnlocked !== 'boolean') {
            this._log.error('Chilly: Received invalid isUnlocked parameter. Please report this bug.');
            return;
        }
