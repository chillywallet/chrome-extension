...
import { NOTIFICATION_NAMES } from '../permissions';
...
export abstract class AbstractStreamProvider extends BaseProvider {
    ...
    constructor(
        ...
        pipeline(
            ...
            connectionStream,
            this._handleStreamDisconnect.bind(this, 'Chilly'),
        );
        ...
        this._jsonRpcConnection = createStreamMiddleware({
            retryOnMessage: 'CHILLY_EXTENSION_CONNECT_CAN_RETRY',
        ...
        pipeline(
            ...
            this._jsonRpcConnection.stream,
            this._handleStreamDisconnect.bind(this, 'Chilly RpcProvider'),
        );
        ...
        this._jsonRpcConnection.events.on('notification', payload => {
            ...
            if (method === NOTIFICATION_NAMES.accountsChanged) {
                this._handleAccountsChanged(params);
            } else if (method === NOTIFICATION_NAMES.unlockStateChanged) {
                this._handleUnlockStateChanged(params);
            } else if (method === NOTIFICATION_NAMES.chainChanged) {
            ...
            } else if (method === 'CHILLY_STREAM_FAILURE') {
     ...
     *
     * Calls MESSAGE_TYPE.GET_PROVIDER_STATE and passes the result to
    ...
    protected async _initializeStateAsync() {
        ...
        try {
            ...
            this._log.error('Chilly: Failed to get initial state. Please report this bug.', error);
        }
    ...
    private _handleStreamDisconnect(streamName: string, error: Error | null) {
        let warningMsg = `Chilly: Lost connection to "${streamName}".`;
        if (error?.stack) {
            warningMsg += `\n${error.stack}`;
        }
...
export class StreamProvider extends AbstractStreamProvider {
     ...
     *
     * Calls `chilly_getProviderState` and passes the result to
     * {@link BaseProvider._initializeState}. Logs an error if getting initial state
     * fails. Throws if called after initialization has completed.
     */
    async initialize() {
