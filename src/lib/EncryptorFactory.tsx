...
import { ExportableKeyEncryptor } from '../controller/KeyringController';
...
const encryptFactory =
    (iterations: number) =>
    async (password: string, data: unknown, key?: EncryptionKey | CryptoKey, salt?: string) =>
...
const keyFromPasswordFactory =
    (iterations: number) =>
    async (password: string, salt: string, exportable?: boolean, opts?: KeyDerivationOptions) =>
        keyFromPassword(
            password,
            salt,
            exportable,
