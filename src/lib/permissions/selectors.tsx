 ...
 */
export const getPermittedAccountsByOrigin = createSelector(getSubjects, subjects => {
    return Object.values(subjects).reduce((originToAccountsMap: any, subject: any) => {
        const caveats = subject.permissions?.eth_accounts?.caveats || [];

        const caveat = caveats.find(
            ({ type }: any) => type === CaveatTypes.restrictReturnedAccounts,
        );
...
export const getChangedAccounts = (
    newAccountsMap: Map<string, string[]>,
    previousAccountsMap: Map<string, string[]>,
) => {
