...
import type { DataWithOptionalCause, JsonRpcError } from '@metamask/rpc-errors';
...
export class ApprovalRequestNotFoundError2 extends Error {
    constructor(origin: string, type: string) {
        super(`Approval request with origin '${origin}' and type '${type}' not found.`);
    }
}
...
> {
    ...
    updateRequestState(opts: UpdateRequestStateOptions): void {
        ...
        this.update(draftState => {
            draftState.pendingApprovals[opts.id].requestState = opts.requestState as never;
        });
    }
    ...
    #add(
        ...
        if (!this.#typesExcludedFromRateLimiting.includes(type) && this.has({ origin, type })) {
            const approval = Object.values(this.state.pendingApprovals).find(
                _approval => _approval.origin === origin && _approval.type === type,
            );

            if (approval) {
                this.#delete(approval.id);
            } else {
                throw new ApprovalRequestNotFoundError2(origin, type);
            }
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            this.#addToStore(id, origin, type, requestData, requestState, expectsResult);
        });
    }
